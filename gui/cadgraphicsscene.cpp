...
void CadGraphicsScene::mousePressEvent(QGraphicsSceneMouseEvent *mouseEvent)
{
    ...
    if(mouseEvent->button() == Qt::LeftButton)
    {
        ...
        {
            int radius = 2;
            QGraphicsEllipseItem * ellipse = this->addEllipse(mouseEvent->scenePos().x() - radius,
                                                              mouseEvent->scenePos().y() - radius, radius*2, radius*2);
            ...
            if (points.size() == 3)
            {
                if (false)
                {
                    ...
                    QGraphicsEllipseItem * ellipse = new QGraphicsEllipseItem(center.x() - rad,
                                                                              center.y() - rad, rad*2, rad*2);
                    if(drawCircle)
